...
def get_image_url(index: str, photo_count: int | None) -> list:
    template_images_url = "https://images.wbstatic.net/c516x688/new/{collection}/{idx}.jpg"

    return [template_images_url.format(collection=str(index)[:4] + "0000", idx=str(index) + f"-{i}")
            for i in range(1, photo_count or 2)]
